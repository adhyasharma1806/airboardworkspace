  ...
  const toggleTracking = () => {
    ...
    toast({
      ...
      description: isTracking
        ? "Hand tracking has been disabled."
        : "Hand tracking is now active.",
    });
  };
  ...
  useEffect(() => {
    ...
    hands.onResults((results: any) => {
      ...
      if (
        ...
        setGesture(detected);
        console.log("🖐️ Gesture Detected:", detected);

        if (detected === "Fist") handleKeyPress("BACKSPACE");
        else if (detected === "Peace") handleKeyPress("SPACE");
        else if (detected === "Point") handleKeyPress("A");
    ...
    <div className="min-h-screen bg-gradient-to-br from-cyber-darker via-cyber-dark to-cyber-light">
      <header className="glass-morphism border-b border-cyber-primary/20 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-xl font-bold cyber-font text-glow">
              AirBoard Workspace
            </h1>
          </div>
      ...
      <div className="flex h-[calc(100vh-80px)]">
        ...
        <div className="w-80 p-6 space-y-6">
          <CameraPreview isTracking={isTracking} handDetected={handDetected} gesture={gesture} />
          <GestureControls />
          {showKeyboard && (
            <VirtualKeyboard
